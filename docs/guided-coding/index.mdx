...
**Guided Learning** is a variant for developers who are still learning the domain a plan touches: in the Implementing Phase, you write the code yourself while the agent teaches you, ideally with a senior developer at your side during Planning and Guiding. See [Guided Learning: Getting Started](./guided-learning-getting-started.mdx).
...
Callouts like this one show my concrete setup. This is the layer the [Guided Coding skills](https://github.com/feO2x/guided-coding) encode, and [Guided Coding: Getting Started](./guided-coding-getting-started.mdx) walks through it end to end.

Everything in these callouts is optional. Nothing here is required to practice Guided Coding.
:::

Please don't read "I installed the skills" as "I adopted Guided Coding". The skills automate bookkeeping - writing the draft, freezing it with a timestamp at the right moment, recording deviations. They cannot do the two things the methodology is actually about: thinking hard about the plan, and reviewing every line of code that comes back. Those remain yours.
...
- Plans are **frozen** when their Planning Phase ends: they receive a UTC timestamp and a `*Frozen at ...*` line, and get committed. Plan Deviations documents are frozen once committed. The only permitted edit to a frozen plan is ticking an acceptance criterion once the implementation actually satisfies it.
...
- **Guided Learning** joins as a variant for developers who want to learn: two skills let you implement a plan yourself while the agent teaches you, and a private learning profile tracks your progress across sessions. See [Guided Learning: Getting Started](./guided-learning-getting-started.mdx).
...
- Want to try the tooling right away? Start with [Guided Coding: Getting Started](./guided-coding-getting-started.mdx).
...
- Want to write the code yourself and learn along the way? Read [Guided Learning: Getting Started](./guided-learning-getting-started.mdx).
